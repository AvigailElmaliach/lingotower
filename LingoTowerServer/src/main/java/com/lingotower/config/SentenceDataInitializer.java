...
public class SentenceDataInitializer implements CommandLineRunner {
	...
	/**
	 * Runs after the application context is loaded to initialize example sentences
	 * from JSON files. It loads sentences and then triggers the translation of any
	...
	/**
	 * Loads example sentences from JSON files matching the provided location
	 * pattern. It reads each file and processes the word-sentence pairs within, and
	 * then updates missing translations.
	 * 
	 * @param locationPattern The pattern to locate JSON sentence files in the
	 *                        classpath.
	 */
	private void loadExampleSentencesFromJson(String locationPattern) {
		try {
			...
			for (Resource sentenceFile : sentenceFiles) {
				...
				processSentenceFileForUpdatingTranslations(sentenceFile); // Update existing sentences with translations
																			// from file
			}
	...
	/**
	 * Processes a single JSON sentence file to load new words and their example
	...
	private void processSentenceFileForLoading(Resource sentenceFile) {
		try {
			List<ExampleSentenceCreateDTO> wordWithSentencesList = objectMapper.readValue(sentenceFile.getInputStream(),
					new TypeReference<List<ExampleSentenceCreateDTO>>() {
					});
	...
	/**
	 * Processes a single JSON sentence file to update existing example sentences in
	 * the database with translations provided in the file.
	 * 
	 * @param sentenceFile The Resource representing the JSON sentence file.
	 */
		...
		try {
			...
			for (ExampleSentenceCreateDTO wordWithSentences : wordWithSentencesList) {
				...
				if (existingWordOptional.isPresent()) {
					...
					if (exampleSentencesFromDto != null && exampleTranslationsFromDto != null
							&& exampleSentencesFromDto.size() == exampleTranslationsFromDto.size()) {
						...
						for (int i = 0; i < exampleSentencesFromDto.size(); i++) {
							...
							for (ExampleSentence existingSentence : existingSentences) {
								if (existingSentence.getSentenceText().equals(sentenceTextFromDto)
										&& (existingSentence.getTranslatedText() == null
												|| existingSentence.getTranslatedText().isEmpty())
										&& translatedTextFromDto != null && !translatedTextFromDto.isEmpty()) {
			...
			System.err.println(
					"    Error updating translations from file " + sentenceFile.getFilename() + ": " + e.getMessage());
		}
	}

	/**
	 * Adds a list of example sentences (with their translations) to a given Word
	 * object. It checks for existing sentences before adding new ones.
	 * 
	 * @param word                The Word object to add sentences to.
	 * @param exampleSentences    The list of example sentences to add.
	 * @param exampleTranslations The list of corresponding translations for the
	 *                            sentences.
	...
	/**
	 * Retrieves the translated text for a sentence. It first checks if a
	 * translation is provided in the exampleTranslations list. If not, it uses the
	 * translation service.
	 * 
	...
	private String getTranslatedText(Word word, String sentenceText, List<String> exampleTranslations, int index) {
		if (exampleTranslations != null && index < exampleTranslations.size() && exampleTranslations.get(index) != null
				&& !exampleTranslations.get(index).isEmpty()) {
			return exampleTranslations.get(index);
		} else {
			try {
				return translationService.translateText(sentenceText, word.getSourceLanguage(),
						word.getTargetLanguage());
			} catch (HttpClientErrorException.TooManyRequests e) {
				System.out.println(
						"Translation API limit reached for sentence: " + sentenceText + ". Translation not available.");
	...
	/**
	 * Checks if a given sentence text already exists in the list of example
	 * sentences for a word. This is used to prevent duplicate entries.
	 * 
